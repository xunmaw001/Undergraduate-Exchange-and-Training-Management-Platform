        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '教师类型' : ''">
                        <el-select v-model="searchForm.jiaoshiTypes" placeholder="请选择教师类型">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xuefenjilu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/benkeshengjiaoliuguanli/upload/xuefenjiluMuBan.xls"
                        >批量导入学分增加记录数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xuefenjilu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xuefenjiluUploadSuccess"
                                :on-error="xuefenjiluUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xuefenjilu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入学分增加记录数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xuefenjilu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xuefenjilu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xuefenjilu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xuefenjilu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xuefenjilu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xuefenjilu',//当前表表名,
                        sumColum : 'xuefenjilu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xuefenjilu_name',//分组字符串字段
                        // types : 'xuefenjilu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['xuefenjiluDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            xuefenjiluUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xuefenjilu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入学分增加记录数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "xuefenjilu/update",
                        method: "post",
                        data: {
                            id:id,
//                            xuefenjiluTypes:xuefenjiluTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
